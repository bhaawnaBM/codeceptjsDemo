...
async function installBrowser(packagePath, browserPath, browser) {
    if (browser.name !== 'firefox')
        return;
    const firefox = new playwright_1.Playwright(packagePath, [browser]).firefox;
    const userDataDir = path.join(browserPath, 'cached-profile');
    await fsMkdirAsync(userDataDir, { recursive: true });
    helper_1.logPolitely('Pre-compiling Firefox scripts at ' + userDataDir);
