...
class ElementHandle extends js.JSHandle {
    ...
    async _retryPointerAction(actionName, action, options = {}) {
        this._page._log(exports.inputLog, `elementHandle.${actionName}()`);
        ...
        throw new errors_1.TimeoutError(`waiting for element to receive pointer events failed: timeout exceeded. Re-run with the DEBUG=pw:input env variable to see the debug log.`);
    }
    async _performPointerAction(actionName, action, deadline, options = {}) {
        ...
        try {
            ...
            await this._page._frameManager.waitForSignalsCreatedBy(async () => {
                ...
                this._page._log(exports.inputLog, `performing "${actionName}" action...`);
                await action(point);
                this._page._log(exports.inputLog, `... "${actionName}" action done`);
                this._page._log(exports.inputLog, 'waiting for scheduled navigations to finish...');
                await this._page._delegate.setActivityPaused(false);
                paused = false;
                if (restoreModifiers)
    ...
    async selectOption(values, options) {
        this._page._log(exports.inputLog, `elementHandle.selectOption(%s)`, values);
        const deadline = this._page._timeoutSettings.computeDeadline(options);
        let vals;
    ...
    async fill(value, options) {
        this._page._log(exports.inputLog, `elementHandle.fill(${value})`);
    ...
    async selectText() {
        this._page._log(exports.inputLog, `elementHandle.selectText()`);
    ...
    async setInputFiles(files, options) {
        this._page._log(exports.inputLog, `elementHandle.setInputFiles(...)`);
        const deadline = this._page._timeoutSettings.computeDeadline(options);
    ...
    async focus() {
        this._page._log(exports.inputLog, `elementHandle.focus()`);
        const injectedResult = await this._evaluateInUtility(({ injected, node }) => injected.focusNode(node), {});
        handleInjectedResult(injectedResult, '');
    }
    async type(text, options) {
        this._page._log(exports.inputLog, `elementHandle.type("${text}")`);
        const deadline = this._page._timeoutSettings.computeDeadline(options);
        await this._page._frameManager.waitForSignalsCreatedBy(async () => {
            await this.focus();
    ...
    async press(key, options) {
        this._page._log(exports.inputLog, `elementHandle.press("${key}")`);
        const deadline = this._page._timeoutSettings.computeDeadline(options);
        await this._page._frameManager.waitForSignalsCreatedBy(async () => {
            await this.focus();
    ...
    async check(options) {
        this._page._log(exports.inputLog, `elementHandle.check()`);
        await this._setChecked(true, options);
    }
    async uncheck(options) {
        this._page._log(exports.inputLog, `elementHandle.uncheck()`);
    ...
    async _waitForDisplayedAtStablePosition(deadline) {
        ...
        }, { rafCount, timeout: helper_1.helper.timeUntilDeadline(deadline) });
...
function handleInjectedResult(injectedResult, timeoutMessage) {
    ...
    if (injectedResult.status === 'timeout')
        throw new errors_1.TimeoutError(`waiting for ${timeoutMessage} failed: timeout exceeded. Re-run with the DEBUG=pw:input env variable to see the debug log.`);
