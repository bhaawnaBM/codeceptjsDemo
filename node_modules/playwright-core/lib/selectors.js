...
class Selectors {
    ...
    _waitForSelectorTask(selector, state, deadline) {
        const parsed = this._parseSelector(selector);
        const task = async (context) => context.evaluateHandleInternal(({ evaluator, parsed, state, timeout }) => {
        ...
        }, { evaluator: await this._prepareEvaluator(context), parsed, state, timeout: helper_1.helper.timeUntilDeadline(deadline) });
